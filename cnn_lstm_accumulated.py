...
for epoch in range(epochs):
  ...
  #=============valid==================
    ...
    if epoch == 0:
        ...
        running_patience = 0
        best_epoch = 0
        torch.save(my_model.to('cpu').state_dict(),  'best-model-parameters-cnn-lstm-accumulate.pt')
        my_model.to(device)
    else:
...
with open('cnn_lstm_accumulated_results.txt', 'a') as f:
    print('===============================================================', file = f)
    print('=====Training results=====', '\n', 'Best valid loss: ', best_valid_loss, '\n', 'Best epoch: ', best_epoch, '\n', 'Total time for training: ', total_training_time, '\n', 'Time per epoch', time_per_epoch, file = f)

criterion_MSE = nn.MSELoss()
criterion_MAPE = MAPE()
my_model.load_state_dict(torch.load('./best-model-parameters-cnn-lstm-accumulate.pt'))
my_model.eval()
my_model.to('cpu')
for sequence, label in test_loader:
    output = my_model(sequence)
    loss_MAE = criterion(output, label)
