...
class Net(nn.Module):
    def __init__(self, embed_dim, num_heads):
        ...
        self.fc3 = nn.Linear(embed_dim, fc_param1)
    ...
    def forward(self, key, query, value):
        ...
        key = F.relu(self.conv1(key))
        query = F.relu(self.conv1(query))
        value = F.relu(self.conv1(value))
        key = F.relu(self.conv2(key))
        query = F.relu(self.conv2(query))
        value = F.relu(self.conv2(value))

        attn_output, attn_output_weights = self.multihead_attn(query, key, value)
        x = self.fc3(attn_output)
        x = self.fc4(x)
        output = self.fc5(x)
        return output
...
# print(type(test_output), type(test_label))
# label = label.detach().numpy()
# print(type(label))
# plt.plot(range(0, len(test_output)), test_output)
# plt.plot(range(0, len(test_label)), test_label)

# plt.plot(range(100), test_output[0:100])
# plt.plot(range(100), test_label[0:100])