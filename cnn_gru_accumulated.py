...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.fc(x).squeeze(-1) #Paul add squeeze on last dim 
        return output
...
for epoch in range(epochs):
  ...
  #=============valid==================
    ...
    if epoch == 0:
        ...
        running_patience = 0
        best_epoch = 0
        torch.save(my_model.to('cpu').state_dict(),  'best-model-parameters-cnn-gru-accumulate.pt')
        my_model.to(device)
    else:
...
with open('cnn_gru_accum_results.txt', 'a') as f:
    print('===============================================================', file = f)
...
criterion_MAPE = MAPE()
my_model.load_state_dict(torch.load('./best-model-parameters-cnn-gru-accumulate.pt'))
my_model.eval()
my_model.to('cpu')
