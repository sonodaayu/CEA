...
vector_length = 24
train_pool_hours = 210 * 24 #day*24h
...
train_list, test_list = train_test(hourly_consumption_list, train_pool_hours, test_pool_hours)

def make_vectors(hourly_consumption_list):
    num_vectors = len(hourly_consumption_list) - vector_length
    vectors = []
    for i in range(num_vectors):
        vector = []
        for j in range(vector_length):
            vector.append(hourly_consumption_list[i + j])
        vectors.append(vector)
    vectors = np.array(vectors)
    labels = []
    for i in range(num_vectors):
        labels.append(hourly_consumption_list[i + vector_length])
...
with open('simple_PVS_hourly_results.txt', 'a') as f:
...
plt.plot(range(500), test_pool_labels[0:500])
plt.plot(range(500), forecasted_values[0:500])
plt.show()